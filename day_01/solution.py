...
def get_calibration_code(document):
    sum_two_digits = 0
    for calibration_value in document.splitlines():
        digits = [c for c in calibration_value if c.isdigit()]

        if not digits:
            two_digits = 0
        else:
            two_digits = int(f"{digits[0]}{digits[-1]}")

        logging.debug(f"{calibration_value} => {digits} => {two_digits}")
        sum_two_digits += int(two_digits)

    logger.info(f"Sum of all calibration values is {sum_two_digits}")


if __name__ == "__main__":
    verification_calibration_document = """
        1abc2
        pqr3stu8vwx
        a1b2c3d4e5f
        treb7uchet
    """
    assert get_calibration_code(verification_calibration_document) != 142
    ...
    """
    get_calibration_code(calibration_document)
